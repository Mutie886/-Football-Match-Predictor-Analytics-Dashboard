...
def predict_match_outcome(home_team, away_team, team_metrics):
    ...
    if strength_diff > 1.0:
        ...
        home_advantage_multiplier = 1.02
        away_penalty = 0.98
    ...
    if home_form and away_form:
        ...
        home_form_score = sum(form_values.get(r, 0) for r in home_form[-3:])
        away_form_score = sum(form_values.get(r, 0) for r in away_form[-3:])
        form_impact = (home_form_score - away_form_score) * 2
    ...
    return {
        ...
        "home_advantage_multiplier": round((home_advantage_multiplier - 1) * 100, 1),
...
def parse_matches(text: str):
    """Simple parsing function that works with clean input"""
